...
for obs_id in "${obs_id_array[@]}"; do
     python /users/wl42/data/wl42/OBS0/calibration/sav2npz.py $obs_id
done


#N=${#good_obs_list[@]}                    
#sbatch -p batch --array=0-$(($N - 1)) --mem=$mem -t $time -n 8 --export=N=$N,poscal=$poscal, noisecheck.sh ${good_obs_list[@]}
#sbatch --account=jpober-condo --array=0-$(($N - 1))%12 --mem=$mem -t $time -n 8 --export=N=$N,poscal=$poscal, noisecheck.sh ${good_obs_list[@]}
